...
class WeiboServiceTest extends WithSpringBootTestAnnotation {
  ...
  @Test
  @Order(6)
  void cacheConsistency1() throws Exception {
    // 先查询一次，确保缓存中有值（否则单飞模式可能会影响测试结果）
    weiboService.getById(MIN_ID);
    int round = 0;
    boolean isDbMatchCache = true;
    while (isDbMatchCache) {
      int threadCount = 2 + RandomUtil.randomInt(19);
      log.info("第 {} 轮 - {} 线程", ++round, threadCount);
        ...
        Thread thread =
            Thread.startVirtualThread(
                () -> {
                  ...
                  Weibo weibo = weiboService.getById(MIN_ID);
                  weibo.setContent(IdUtil.nanoId());
                  weiboService.updateDeprecated(weibo);
                });
        threadList.add(thread);
        countDownLatch.countDown();
      }
      joinThreads(threadList);
      String dbContent = weiboMapper.selectById(MIN_ID).getContent();
      String cacheContent = weiboService.getById(MIN_ID).getContent();
      isDbMatchCache = dbContent.equals(cacheContent);
    }
    // 验证缓存和数据库不一致（由于代码存在缺陷，我们期待的就是不一致）
    Assertions.assertFalse(isDbMatchCache);
    // 还原内容
    Weibo db = weiboMapper.selectById(MIN_ID);
    db.setContent("用户1的微博1");
    weiboMapper.updateById(db);
  }

  @Test
  @Order(6)
  void cacheConsistency2() throws Exception {
    DateTime deadline = DateUtil.offsetMinute(new Date(), 5);
    // 先查询一次，确保缓存中有值（否则单飞模式可能会影响测试结果）
    weiboService.getById(MIN_ID);
    int round = 0;
    boolean isDbMatchCache = true;
    while (isDbMatchCache && DateUtil.compare(new Date(), deadline) <= 0) {
      int threadCount = 2 + RandomUtil.randomInt(19);
      log.info("第 {} 轮 - {} 线程", ++round, threadCount);
      CountDownLatch countDownLatch = new CountDownLatch(threadCount);
      List<Thread> threadList = new ArrayList<>();
    ...
    // 验证至少在测试的时间段里，缓存和数据库是一致的
    Assertions.assertTrue(isDbMatchCache);
    // 还原内容
    Weibo db = weiboMapper.selectById(MIN_ID);
    db.setContent("用户1的微博1");
    weiboMapper.updateById(db);
